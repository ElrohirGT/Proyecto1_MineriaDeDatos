...
if (nrow(datos) > 0) {
  ...
  ggplot(datos, aes(x = id)) +
    geom_histogram(aes(y = ..density..), bins = 30, fill = "skyblue", color = "black") +
    stat_function(fun = dnorm, args = list(mean = mean(datos$id, na.rm = TRUE),
                                           sd = sd(datos$id, na.rm = TRUE)),
                  color = "red", size = 1) +
    ggtitle("Histograma para Id")

  qqnorm(datos$id, main = "QQ Plot de Id")
  qqline(datos$id, col = "red", lwd = 2)
...
datos <- datos[!is.na(datos$productionCountriesAmount), ]

if (nrow(datos) > 0) {
  
  ggplot(datos, aes(x = productionCountriesAmount)) +
    geom_histogram(aes(y = ..density..), bins = 30, fill = "skyblue", color = "black") +
    stat_function(fun = dnorm, args = list(mean = mean(datos$productionCountriesAmount, na.rm = TRUE),
                                           sd = sd(datos$productionCountriesAmount, na.rm = TRUE)),
                  color = "red", size = 1) +
    ggtitle("Histograma para productionCountriesAmount")

  qqnorm(datos$productionCountriesAmount, main = "QQ Plot de productionCountriesAmount")
  qqline(datos$productionCountriesAmount, col = "red", lwd = 2)
...
datos <- datos[!is.na(datos$actorsPopularity), ]

if (nrow(datos) > 0) {
  
  ggplot(datos, aes(x = actorsPopularity)) +
    geom_histogram(aes(y = ..density..), bins = 30, fill = "skyblue", color = "black") +
    stat_function(fun = dnorm, args = list(mean = mean(datos$actorsPopularity, na.rm = TRUE),
                                           sd = sd(datos$actorsPopularity, na.rm = TRUE)),
                  color = "red", size = 1) +
    ggtitle("Histograma para actorsPopularity")

  qqnorm(datos$actorsPopularity, main = "QQ Plot de actorsPopularity")
  qqline(datos$actorsPopularity, col = "red", lwd = 2)
...
datos <- datos[!is.na(datos$castWomenAmount), ]

if (nrow(datos) > 0) {
  
  ggplot(datos, aes(x = castWomenAmount)) +
    geom_histogram(aes(y = ..density..), bins = 30, fill = "skyblue", color = "black") +
    stat_function(fun = dnorm, args = list(mean = mean(datos$castWomenAmount, na.rm = TRUE),
                                           sd = sd(datos$castWomenAmount, na.rm = TRUE)),
                  color = "red", size = 1) +
    ggtitle("Histograma para castWomanAmount")

  qqnorm(datos$castWomenAmount, main = "QQ Plot de castWomenAmount")
  qqline(datos$castWomenAmount, col = "red", lwd = 2)
...
datos <- datos[!is.na(datos$castMenAmount), ]

if (nrow(datos) > 0) {
  
  ggplot(datos, aes(x = castMenAmount)) +
    geom_histogram(aes(y = ..density..), bins = 30, fill = "skyblue", color = "black") +
    stat_function(fun = dnorm, args = list(mean = mean(datos$castMenAmount, na.rm = TRUE),
                                           sd = sd(datos$castMenAmount, na.rm = TRUE)),
                  color = "red", size = 1) +
    ggtitle("Histograma para castMenAmount")

  qqnorm(datos$castMenAmount, main = "QQ Plot de castMenAmount")
  qqline(datos$castMenAmount, col = "red", lwd = 2)
...
success <- datos[datos$revenue > budget * 3]
success <- success %>% 
  mutate(popularidad_media = sapply(strsplit(as.character(actorsPopularity), "\\|"),
...
```{r Pregunta Extra 4}
datos$year <- format(datos$releaseDate, "%Y")
datos <- datos[datos$budget > 0]
budget_year <- datos %>% group_by(year) %>% summarise(presupuesto_promedio = mean(budget))
...
```{r Pregunta Extra 5}
datos$year <- format(datos$releaseDate, "%Y")
datos <- datos[order(datos$budget, decreasing = TRUE),]
budget_year <- head(datos[, c('year','budget', 'title')],10)
...
datos <- datos %>%
  mutate(productionCompany = str_squish(productionCompany)) %>%  # Elimina espacios extra dentro del string
  separate_rows(productionCompany, sep = "\\|") %>%  # Separa en filas individuales
  mutate(productionCompany = str_trim(productionCompany))  # Elimina espacios extra en cada nombre

companies <- datos %>% group_by(productionCompany) %>% summarise(ingresos = sum(revenue, na.rm = TRUE), total_peliculas = n())
...
companies
ggplot(companies, aes(x=`Compañia productora`, y=ingresos, fill = `Compañia productora`)) + geom_bar(stat="identity") +
  scale_fill_manual(values = c("#1F77B4", "#FF7F0E", "#2CA02C", "#D62728", "#9467BD", 
             "#8C564B", "#E377C2", "#7F7F7F", "#BCBD22", "#17BECF")) +
