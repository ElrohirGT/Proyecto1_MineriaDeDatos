...
splitted_genres <- strsplit(movies_by_date$genres, split = "|", fixed = TRUE)
movie_genres <- data.frame("", "", 0)
titles <- c("TITLE", "GENRE", "REVENUE")
names(movie_genres)<- titles
for (idx in 1:dim(movies_by_date)[1]) {
  for (genre in splitted_genres[[idx]]) {
    temp <- c(movies_by_date$title[idx], genre, movies_by_date$revenue[idx])
    names(temp) <- titles
    movie_genres<- rbind(movie_genres, temp)
  }
}
movie_genres <- aggregate(as.numeric(movie_genres$REVENUE), by=list(GENRE=movie_genres$GENRE), FUN=sum)
names(movie_genres) <- c("GENRE", "REVENUE")
vec <- movie_genres$REVENUE
names(vec) <- movie_genres$GENRE
barplot(vec, main="Ingresos por género", las=3, col=c("#69b3a2", "#69b300", "#69bfff"))

ordered <- movie_genres[order(movie_genres$REVENUE, decreasing = TRUE),]
```

Como se puede ver en la gráfica, el género de película que mayores ingresos genera es Aventura seguido muy cercanamente de Acción con $`r max(movie_genres$REVENUE)` y $`r ordered$REVENUE[2]` respectivamente.
...
```{r Pregunta Extra 2}
splitted_countries <- strsplit(movies_by_date$productionCountry, split = "|", fixed = TRUE)
movie_genres <- data.frame("", "")
titles <- c("TITLE", "COUNTRY")
names(movie_genres)<- titles
for (idx in 1:dim(movies_by_date)[1]) {
  for (country in splitted_countries[[idx]]) {
    temp <- c(movies_by_date$title[idx], country)
    names(temp) <- titles
    movie_genres<- rbind(movie_genres, temp)
  }
}
movie_genres <- aggregate(movie_genres[,c("COUNTRY")], by=list(movie_genres$COUNTRY), FUN="length")
names(movie_genres) <- c("COUNTRY", "COUNT")
movie_genres <- movie_genres[order(movie_genres$COUNT, decreasing = TRUE),]
movie_genres <- head(movie_genres, n=3)

vec <- movie_genres$COUNT
names(vec) <- movie_genres$COUNTRY
barplot(vec, main="Top 3 países productores de películas", las=1, col=c("#69b3a2", "#69b300", "#69bfff"))
```

Como se puede ver, el país que produce la mayor cantidad de películas es los Estados Unidos de América y por mucho, ya que en total ha producido `r max(movie_genres$COUNT)` películas.

### ¿Los directores más populares suelen generar películas con mayor éxito de taquilla o mejor calificación?
